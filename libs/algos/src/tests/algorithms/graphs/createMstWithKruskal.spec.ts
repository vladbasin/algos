import { AdjacencyListGraph, createMstWithKruskal, GraphEdge } from '../../../lib';

describe('createMstWithKruskal', () => {
  it('should return an empty graph when input graph is empty', () => {
    const graph = new AdjacencyListGraph(0);
    const mst = createMstWithKruskal(graph);
  ...
  it('should fail if graph is not connected', () => {
    ...
    expect(() => createMstWithKruskal(graph)).toThrow('Graph is not connected');
  });
  ...
  it('should handle multiple connected components', () => {
    ...
    graph.add(new GraphEdge(1, 2, 2));
    graph.add(new GraphEdge(2, 1, 2)); // Reverse edge
    graph.add(new GraphEdge(3, 4, 3));
    graph.add(new GraphEdge(4, 3, 3)); // Reverse edge
    graph.add(new GraphEdge(4, 5, 4));
    graph.add(new GraphEdge(5, 4, 4)); // Reverse edge

    const mst = createMstWithKruskal(graph, true);
    const edges = Array.from(mst.iterateAllEdges());
    ...
    expect(edges).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ from: 0, to: 1, weight: 1 }),
        expect.objectContaining({ from: 1, to: 2, weight: 2 }),
        expect.objectContaining({ from: 3, to: 4, weight: 3 }),
        expect.objectContaining({ from: 4, to: 5, weight: 4 }),
      ])
    );
  });
