import { AdjacencyMatrixGraph, GraphEdge } from '../../../lib';

describe('AdjacencyMatrixGraph', () => {
  let graph: AdjacencyMatrixGraph;

  beforeEach(() => {
    graph = new AdjacencyMatrixGraph(5); // Initialize with a size of 5 for testing
  });
  ...
  describe('add', () => {
    ...
    it('should override an existing edge', () => {
      graph.add(new GraphEdge(0, 1, 10));
      graph.add(new GraphEdge(0, 1, 20));
      const edges = Array.from(graph.iterateEdgesFrom(0));
      expect(edges).toHaveLength(1);
      expect(edges[0]).toEqual(new GraphEdge(0, 1, 20));
    });
    ...
    it('should allow adding an edge with a negative weight', () => {
      graph.add(new GraphEdge(0, 1, -5));
      const edges = Array.from(graph.iterateEdgesFrom(0));
      expect(edges).toHaveLength(1);
      expect(edges[0]).toEqual(new GraphEdge(0, 1, -5));
    });

    it('should add a self-loop edge', () => {
    ...
    it('should add an edge to the maximum valid vertex index', () => {
      graph.add(new GraphEdge(4, 4, 25));
      const edges = Array.from(graph.iterateEdgesFrom(4));
      expect(edges).toHaveLength(1);
      expect(edges[0]).toEqual(new GraphEdge(4, 4, 25));
    });
  });
  ...
  describe('iterateEdgesFrom', () => {
    ...
    it('should iterate correctly when there are no edges in the graph', () => {
      for (let i = 0; i < graph.size; i++) {
        expect(Array.from(graph.iterateEdgesFrom(i))).toHaveLength(0);
      }
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle a graph of size 0', () => {
      const emptyGraph = new AdjacencyMatrixGraph(0);
    ...
    it('should handle adding an edge to a vertex that already has multiple edges', () => {
      ...
      graph.add(new GraphEdge(0, 2, 20));
      graph.add(new GraphEdge(0, 3, 30));
      graph.add(new GraphEdge(0, 2, 25)); // Modify an existing edge

      const edges = Array.from(graph.iterateEdgesFrom(0));
      expect(edges).toHaveLength(3);
      expect(edges).toEqual([new GraphEdge(0, 1, 10), new GraphEdge(0, 2, 25), new GraphEdge(0, 3, 30)]);
    });
  });
});
